...
### Security
- Bump Go toolchain to 1.26.5 to patch **GO-2026-5856** (crypto/tls), reported by
  govulncheck / `make ci`.

### Added
- First multi-arch container image publishing to GHCR
  (`ghcr.io/fjacquet/m365_licenses_exporter`, linux/amd64 + linux/arm64) via a
  GoReleaser `dockers_v2` block with per-image CycloneDX SBOM and OCI labels.
